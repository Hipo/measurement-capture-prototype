...
struct PredictionZone {
    let predictionRect: CGRect
    let pointType: PredictedPointType
    let errorView = ZoneErrorView(frame: .zero)
    
    func contains(point: CGPoint) -> Bool {
        return predictionRect.contains(point)
    }
}
